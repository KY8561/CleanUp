...
public class ReservationService {
	...
	private String MakeHtmlLaundry(List<N_laundry> llist) {
		...
		for(int i=0;i<llist.size();i++){
			...
			sb.append("<td><a href='./reservationList?nlcode="+n.getNl_code()+"'><input type='button' value='예약상세보기'></a></td>");
			sb.append("</tr>");
		}
		sb.append("</table>");
		if(llist.size()!=0){
			if(page<=1){
				sb.append("<a href='#' style='margin-left:35%'>[이전]</a>");
			}else{
				sb.append("<a href='./ReservationShop?page="+(page-1)+"'>[이전]</a>");
			}
	...
	private String selectListHtml(List<Reservation> rlist, List<Person> plist, List<Member> mlist,List<Orderlist> olist, List<Laundryhandle> hlist, String nlcode, int state1) {
		...
		if(rlist.size()!=0){
			if(page<=1){
				...
				sb.append("<a href='./selectState?page="+(page-1)+"&nlcode="+nlcode+"&text="+state1+"'>[이전]</a>");
			}
			for(int a=startpage; a<=endpage; a++){
				if(a==page){
					sb.append("<a href='#'>["+a+"]</a>");
	...
	private String reservaionListHtml(List<Reservation> rlist, List<Person> plist, List<Member> mlist, List<Orderlist> olist, List<Laundryhandle> hlist, String nlcode) {
		...
		if(rlist.size()!=0){
			if(page<=1){
				...
				sb.append("<a href='./reservationList?page="+(page-1)+"&nlcode="+nlcode+"'>[이전]</a>");
			}
			for(int a=startpage; a<=endpage; a++){
				if(a==page){
					sb.append("<a href='#'>["+a+"]</a>");
	...
	private String personReservationHtml(List<Reservation> rlist) {
		...
		for(int i=0; i<rlist.size(); i++){
			...
			sb.append("<table>");
			sb.append("------------------------------------------------------------");
			sb.append("<tr><td>예약일자"+r.getNr_date()+"</td><td>예약번호"+r.getNr_code()+"</td></tr>");
			sb.append("------------------------------------------------------------");
		...
		if(rlist.size()!=0){
			if(page<=1){
				...
				sb.append("<a href='./personReservation?page="+(page-1)+"'>[이전]</a>");
			}
			for(int a=startpage; a<=endpage; a++){
				if(a==page){
					sb.append("<a href='#'>["+a+"]</a>");
