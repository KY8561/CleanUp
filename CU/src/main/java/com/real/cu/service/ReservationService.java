...
/*	지우기2 
  	private void givetakeBtn(N_laundry nl) {
		...
		mav.setViewName("ShopDetailCart4");
	...
	private void selectState() {
		mav = new ModelAndView();
		int i = Integer.parseInt(request.getParameter("text"));
		String id="aaa";									// 세션에서 불러왔다고 친다
		List<N_laundry> llist = rDao.getshop(id); 			//로그인한 사장의 세탁소정보를 가져온다
		String code = getlist(llist); 						//세탁소정보중 코드만 가져온다.
		List<Reservation> rlist=rDao.reservationList(code); //세탁소코드로 그세탁소의 예약내역을 가져온다.
		List<Person> plist = getperson(rlist);				//person리스트를 가져온다
		List<Member> mlist = getmember(plist);
		List<Orderlist> olist = getorder(rlist);
		String List = stateshowHtml(rlist,plist,mlist,olist,i);
		mav.addObject("RLIST",List);
		mav.setViewName("ReservationInfo");
	}

	private String stateshowHtml(List<Reservation> rlist, List<Person> plist, List<Member> mlist, List<Orderlist> olist, int state) {
		StringBuilder sb=new StringBuilder();
		Reservation r = new Reservation();
		Person p = new Person();
		Member m = new Member();
		Orderlist o = new Orderlist();
		for(int i=1; i-1<plist.size(); i++){
			r=rlist.get(i-1);
			p=plist.get(i-1);
			m=mlist.get(i-1);
			if(r.getNr_state()==state){
				sb.append("<tr><td><input type='checkbox' name='check' value='"+r.getNr_code()+"'></td>");
				sb.append("<td>"+r.getNr_pid()+"</td>");
				sb.append("<td>"+p.getP_phone()+"</td>");
				sb.append("<td>"+o.getO_qty()+"</td>");
				sb.append("<td>"+m.getM_address()+"</td>");
				sb.append("<td>"+r.getNr_state()+"</td>");
			}
		}
		return sb.toString();
	}
	...
	private void reservationList() {
		...
		String id="ddd";									// 사업자아이디세션에서 불러왔다고 친다
		List<N_laundry> llist = rDao.getshop(id); 			//로그인한 사장의 세탁소정보를 가져온다
		List<Reservation> rlist=getreservationcode(llist); //세탁소코드로 그세탁소의 예약내역을 가져온다.
		List<Person> plist = getperson(rlist);				//person리스트를 가져온다
		List<Member> mlist = getmember(plist);
		List<Orderlist> olist = getorder(rlist);
		List<Laundryhandle> hlist = gethandle(olist);
		System.out.println("오리스트"+olist);
		System.out.println(hlist);
		String List = reservaionListHtml(rlist,plist,mlist,olist);	
	...
	private List<Reservation> getreservationcode(List<N_laundry> llist) {
		...
		for(int i=0;i<llist.size();i++){
			l = llist.get(i);
			System.out.println(l.getNl_code());
	...
	private String reservaionListHtml(List<Reservation> rlist, List<Person> plist, List<Member> mlist, List<Orderlist> olist) {
		...
		for(int i=1; i-1<plist.size(); i++){
			...
			p=plist.get(i-1);
			m=mlist.get(i-1);
			if(r.getNr_pid().equals(p.getP_id())){
				sb.append("<tr><td><input type='checkbox' name='check' value='"+r.getNr_code()+"'></td>");
				sb.append("<td>"+r.getNr_pid()+"</td>");
				sb.append("<td>"+p.getP_phone()+"</td>");
				sb.append("<td>"+o.getO_lhcode()+"</td>");
				sb.append("<td>"+m.getM_address()+"</td>");
	...
	private void givetakeSelect(N_laundry nl) {
		...
		String NLCODE=session.getAttribute("NLCODE").toString();
		System.out.println(NLCODE+"giveANDtake");
		...
		//	int[] moneyList = showMoneyHtml(lList,QTY,NUM,All); 
	...
	private void coinPayComplete() {
		...
		//	String BID = "ddd";			//사업자아이디
		String PID ="aaa"; 			//회원아이디 세션에서 가져왔다고 친다.
		int point = (int)session.getAttribute("ALL");				//포인트
		...
		//예약
		r.setNr_deliverydate("DELIVERYDATE");
		r.setNr_pickupdate("PICKUPDATE");
		r.setNr_deliverytime("DELIVERY");
		r.setNr_pickuptime("PICKUP");
		r.setNr_nlcode(NLCODE);
		r.setNr_pid(PID);

		rDao.insertReservation(r);  //예약하기 insert
